...
options_init_from_torrc(int argc, char **argv)
{
  ...
  const config_line_t* pRootFolderOption = config_line_find(cmdline_only_options, "--root-home-folder");
  if (pRootFolderOption) {
    const char *homeDir = getenv("HOME");
    char fullPath[500] = "";
    strcpy(fullPath, homeDir);
    char *pLib = strstr(fullPath, "/Library");
    if(pLib) {
      *pLib = '\0';
    }
    int lastCharHomeDir = strlen(homeDir) - 1;
    int lastCharOffsetPath = strlen(pRootFolderOption->value) - 1;
    if (homeDir[lastCharHomeDir]!= '\\' && homeDir[lastCharHomeDir]!= '/'
        && pRootFolderOption->value[lastCharOffsetPath]!= '\\' && pRootFolderOption->value[lastCharOffsetPath]!= '/' ) {
          strcat(fullPath, "/");
    }
    strcat(fullPath, pRootFolderOption->value);
    
    log_info(LD_CONFIG, "User home: %s\n", homeDir);
    log_info(LD_CONFIG, "Root folder offset: %s\n", pRootFolderOption->value);
    log_info(LD_CONFIG, "Full current path: %s\n", fullPath);
    chdir(fullPath); 
  }
