...
choose_good_exit_server_general(router_crn_flags_t flags)
{
  ...
  log_info(LD_CIRC, "Additional conditions: uptime %d, capacity %d", need_uptime, need_capacity);
...
//         n_pending_connections);
  ...
  SMARTLIST_FOREACH_BEGIN(the_nodes, const node_t *, node) {
    ...
    if (router_digest_is_me(node->identity)) {
      n_supported[i] = -1;
      log_info(LD_CIRC,"Skipping node %s (%d) -- it's me.", node->identity, i);
//      log_fn(LOG_DEBUG,"Skipping node %s -- it's me.", router->nickname);
    ...
    if (!router_can_choose_node(node, flags)) {
      n_supported[i] = -1;
      log_info(LD_CIRC,"Skipping node %s (%d) -- not choosable.", node->identity, i);
      continue;
    }
    if (node->is_bad_exit) {
      n_supported[i] = -1;
      log_info(LD_CIRC,"Skipping node %s (%d) -- marked as bad/down.", node->identity, i);
      continue; /* skip routers that are known to be down or bad exits */
    }
    if (routerset_contains_node(options->ExcludeExitNodesUnion_, node)) {
      n_supported[i] = -1;
      log_info(LD_CIRC,"Skipping node %s (%d) -- user rejected.", node->identity, i);
      continue; /* user asked us not to use it, no matter what */
    }
    if (options->ExitNodes &&
        !routerset_contains_node(options->ExitNodes, node)) {
      n_supported[i] = -1;
      log_info(LD_CIRC,"Skipping node %s (%d) -- not in list.", node->identity, i);
      continue; /* not one of our chosen exit nodes */
    }
    if (node_exit_policy_rejects_all(node)) {
      n_supported[i] = -1;
      log_info(LD_CIRC,"Skipping node %s (%d) -- it rejects all.", node->identity, i);
